...
2026-09-22 候选浏览器复验已完成：同一 `0.29.0` 构建在 Chromium、Firefox、WebKit 运行 smoke、交互、布局、焦点和性能套件均通过，Chromium 另通过 CDP 触控检查；结果见 [候选复验记录](acceptance/browser-rerun-2026-09-22/README.md)。这更新了自动化候选证据，不代表正式 Safari、原生 IME、实体触控、屏幕阅读器、跨机器复现或商业许可证门槛已通过。此前 npm 首次发布仍未完成；本地安装包验证不等于注册表发布成功。
...
0.29 阶段：Canvas 编辑会话增加工作簿/工作表/数据与修订所有权，忽略迟到回调；即时失焦读取最新草稿，输入法 229 确认键不提交。真实浏览器验收未执行。

0.29 依赖证据进展：SDK 实际构建输入与分包哈希随包输出并安装校验；新增 dayjs 三个 source-map 文件与官方 npm 归档的逐字证据，未解决预打包组件降至 46 个。dayjs 字节同时匹配多个相邻版本，因此不把它当作历史版本唯一证明；商业许可门槛仍未通过。

0.29 导入完整性：拒绝同一单元格的多个地址别名与重复工作表 ID，避免静默覆盖数据或改变验证规则归属。全量 1,145 项测试、构建/API/安装包检查通过，真实浏览器与许可门槛继续未完成。
