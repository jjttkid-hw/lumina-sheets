...
2026-09-23 当前候选更新：提交 `9b6e239` 修复报表示例点击“应用”后的公式栏即时刷新，并增加 `=SUM(1,2,3)` 回归覆盖。当前本地验证为 2,306 项测试通过，格式、API、SDK、站点运行时、严格许可证和可重复构建门禁均通过；可重复构建摘要为站点 `82bb01e5ae9d0d487bca3bfa31b3d9ea4aa017fbace97e135b4dd8a2ceee37b2`、SDK `b7f9637c41454e83b0b4248303f6ab520afacb3d3d8e5d16e5d1d987343c5951`。随后新增的账本提交使本地 `main` 领先 `origin/main` 两个提交，GitHub 推送因连接超时尚未确认；npm 注册表仍未发布，不能据此宣布 v1.0.0。
