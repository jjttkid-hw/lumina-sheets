...
2026-09-23 当前候选更新：提交 `2a1ecc7` 汇总了 Safari 线上公式输入、分页切换和完整报表 CSV 下载证据；当前本地验证为 2,306 项测试通过，格式、API、SDK、站点运行时、严格许可证和可重复构建门禁均通过。GitHub CI `35781252540` 与 CD `35781533802` 均成功，`main` 已与 `origin/main` 同步；npm 注册表仍未发布，不能据此宣布 v1.0.0。
