...
| 浏览器验收 | 三引擎各 24 组与新增各 6 组布局检查通过，完整矩阵待验 | 实际浏览器 Canvas 绘制、编辑/键盘/剪贴板、列表规则、缩放/窄屏、保存刷新、SDK 销毁和导出；记录浏览器与版本 |
| 性能回归 | 三引擎实存数据测量已归档，候选预算与业务语料待验 | 已有操作次数及基准；需要版本绑定的代表性数据、实际环境和原始结果，不能用逻辑行数替代实存格负载 |
...
2026-09-20 浏览器工具自动审批服务返回 503，读取未执行。真实浏览器验收保持未完成，不绕过审批，不以 hooks/DOM 替身测试代替。此前 npm 账号授权与首次发布未完成；安装包本地验证成功不等于注册表发布成功。
...
0.29 依赖证据进展：SDK 实际构建输入与分包哈希随包输出并安装校验；上游版本/许可取证受网络及审批服务故障影响，商业许可门槛未通过。
