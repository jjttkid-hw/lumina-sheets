    ...
    next.onmessage = (event: MessageEvent<RangeBenchmarkResponse>) => {
      ...
      if (message.type === 'progress')
        ...
        if (message.type === 'result') {
          ...
          setStatus('30 次计算结果均与全量重算一致，原始样本可导出。');
  ...
  function download() {
    ...
    const data = {
      ...
      notes: [
        ...
        '增量读取与全量重算都读取全部 10,000 个公式；基线为同一引擎清空结果缓存后重算，不是第三方产品。',
        '每轮先增量后全量，顺序固定，未随机化；30 次来自同一数据集，不代表跨设备 SLA。',
    ...
    <details className="performance-range-panel">
      ...
      {result && (
        <div className="performance-range-results">
          ...
          <span>
            平均实际重算 <b>{result.summary.meanRecomputedFormulas.toLocaleString()} / 10,000</b>
          </span>
          <span>
